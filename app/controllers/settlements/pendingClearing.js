...
var app = angular.module('myApp.pendingClearing', ['ngRoute']);

app.config(['$routeProvider', function($routeProvider) {
    $routeProvider.when('/pendingClearing', {
        templateUrl: 'settlement/pendingClearing.html',
        controller: 'pendingClearingCtrl',
        resolve:
            {
...
app.controller('pendingClearingCtrl', ['$scope','$sessionStorage','apiFactory','$route','$rootScope','$timeout','$filter',function($scope,$sessionStorage,apiFactory,$route,$rootScope,$timeout,$filter) {
    ...
    $rootScope.filtered = $sessionStorage.filtered;
        $scope.info ={};
        $scope.info.title = 'Pending Clearing';
    ...
    var updateView = function () {
        ...
        apiFactory.getSettlementStatus(input).then(function (response) {
            ...
            if (response.data.length === 0){
                ...
                $scope.details = $filter('cheque_filter')(response.data);
            }
